	...
	for (var key in _from) {
		if (!_from.hasOwnProperty(key)) {
			if (typeof _from[key] == 'object') {
				//区分数组还是普通对象
				//新开辟一块内存对象
				to[key] = (_from[key].constructor === Array) ? [] : {};
				deepCopy(to[key], _from[key]);
			} else {
				to[key] = _from[key];
			}
		}
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		init: function(element, options) {
			...
			this._super(element, $.extend(true, {
				...
				scrollTime: 1000, //
				startX: false, //初始化是滚动至x
				slideTime: 0, //滑动动画时间
				snap: SELECTOR_SLIDER_ITEM //元素类名
			}, options));
		...
		_handleTabShow: function(e) {
			console.log("_handleTabShow");
			var self = this;
			self.gotoItem((e.detail.tabNumber || 0), self.options.slideTime);
		},
		_handleIndicatorTap: function(event) {
			console.log("_handleIndicatorTap");
			var self = this;
